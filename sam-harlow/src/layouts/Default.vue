<template>
  <div class="layout">
    <header class="header">
      <g-link to="/contact"
        ><g-image src="@/assets/images/contact.png" width="75px"
      /></g-link>

      <g-link to="/"
        ><svg
          class="logo"
          version="1.0"
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 1028.000000 822.000000"
          preserveAspectRatio="xMidYMid meet"
        >
          <g
            transform="translate(0.000000,822.000000) scale(0.100000,-0.100000)"
            stroke="none"
          >
            <path
              d="M8238 7480 c-208 -51 -317 -171 -370 -404 -17 -77 -21 -136 -25 -386
-7 -435 13 -597 89 -711 64 -95 168 -173 393 -294 260 -140 370 -231 394 -323
6 -22 11 -771 13 -1847 3 -1925 4 -1884 -43 -1961 -26 -43 -66 -57 -163 -57
-123 0 -158 27 -185 143 -7 29 -11 295 -11 721 0 372 -3 1183 -7 1802 l-6
1127 -240 0 -240 0 7 -1162 c8 -1414 24 -2620 36 -2746 16 -170 105 -307 241
-373 96 -47 177 -59 401 -59 305 1 420 32 533 146 86 87 133 208 170 434 13
83 15 336 15 2025 0 1854 -1 1934 -19 2022 -48 230 -121 303 -496 498 -248
130 -354 212 -374 289 -30 121 -22 443 14 533 22 54 51 72 128 80 77 7 138 -9
180 -46 l31 -29 10 -236 9 -236 234 0 234 0 -3 348 c-4 323 -5 351 -25 409
-57 163 -160 251 -348 294 -116 27 -466 26 -577 -1z"
            />
            <path
              d="M1040 4200 l0 -3250 275 0 275 0 0 2320 0 2320 220 0 220 0 0 -2320
0 -2320 275 0 275 0 0 3250 0 3250 -275 0 -275 0 0 -665 0 -665 -220 0 -220 0
0 665 0 665 -275 0 -275 0 0 -3250z"
            />
            <path
              d="M2830 7160 l0 -290 775 0 775 0 0 290 0 290 -775 0 -775 0 0 -290z"
            />
            <path
              d="M4630 4200 l0 -3250 270 0 270 0 0 2281 0 2282 88 -5 c48 -3 99 -11
114 -18 60 -28 94 -176 130 -565 25 -274 35 -1063 37 -2872 l1 -1103 289 0
289 0 -5 1293 c-2 710 -8 1558 -13 1882 -8 530 -11 615 -34 835 -50 465 -91
629 -183 731 -20 22 -34 41 -32 43 2 2 29 19 59 38 66 42 81 58 114 123 51
102 67 221 73 545 8 414 -13 594 -84 748 -39 83 -141 181 -218 211 -122 46
-187 51 -692 51 l-473 0 0 -3250z m860 2673 c49 -55 65 -153 64 -393 -1 -278
-23 -400 -84 -455 -19 -18 -39 -20 -161 -23 l-139 -4 0 457 0 456 146 -3 146
-3 28 -32z"
            />
            <path
              d="M6410 4770 l0 -2680 590 0 590 0 0 290 0 290 -320 0 -320 0 0 2390 0
2390 -270 0 -270 0 0 -2680z"
            />
            <path
              d="M3206 6333 c-162 -49 -307 -229 -353 -438 -17 -75 -18 -233 -21
-2512 l-2 -2433 265 0 265 0 0 1775 0 1775 245 0 245 0 0 -1775 0 -1775 265 0
265 0 0 2398 c0 2592 1 2521 -53 2657 -63 160 -198 296 -329 331 -79 21 -722
19 -792 -3z m477 -376 c41 -18 98 -81 134 -149 l28 -53 5 -373 5 -372 -247 0
-248 0 0 336 c0 357 5 409 51 492 27 49 74 97 113 116 39 19 116 20 159 3z"
            />
            <path
              d="M6410 1255 l0 -285 590 0 590 0 0 285 0 285 -590 0 -590 0 0 -285z"
            />
          </g></svg
      ></g-link>

      <g-link to="/contact"
        ><g-image src="@/assets/images/coverart.png" width="75px"
      /></g-link>

      <!-- <g-link to="/">{{ $static.metadata.siteName }}</g-link> -->
      <nav class="nav"></nav>
    </header>
    <slot />
    <footer>
      <div class="socials">
        <g-link to="https://www.instagram.com/harls_98/">
          <svg viewBox="0 0 31.002 31.001">
            <g>
              <path
                d="M30.988,3.229C30.798,1.383,29.268,0.001,27.409,0C19.473,0,11.538,0,3.602,0.002c-0.217,0-0.438,0.016-0.648,0.057
		C1.196,0.395,0.002,1.849,0.002,3.638c0,3.956,0,7.912,0,11.868c0,3.948,0,7.896,0.002,11.844c0,0.201,0.015,0.403,0.04,0.604
		c0.23,1.717,1.755,3.048,3.49,3.049c7.979,0,15.959,0,23.938-0.001c0.137,0,0.272-0.008,0.409-0.023
		c1.753-0.206,3.116-1.737,3.116-3.502C31,19.496,31,11.516,30.998,3.536C31,3.432,31.002,3.331,30.988,3.229z M15.501,9.542
		c3.294-0.001,5.96,2.661,5.964,5.958c0.004,3.295-2.658,5.962-5.954,5.964c-3.302,0.001-5.972-2.668-5.968-5.968
		C9.547,12.21,12.215,9.542,15.501,9.542z M27.432,13.281c0,4.281,0,8.563,0,12.847c0,0.806-0.49,1.301-1.297,1.301
		c-6.623,0-13.246,0-19.87,0c-0.477,0-0.951,0.001-1.429,0c-0.747-0.001-1.254-0.509-1.254-1.255c-0.001-4.302,0-8.604,0-12.907
		c0-0.043,0-0.087,0-0.138c0.901,0,1.791,0,2.69,0c-0.602,2.429-0.326,4.753,0.863,6.951c0.869,1.604,2.115,2.851,3.704,3.748
		c3.138,1.771,7.042,1.604,10.03-0.438c1.658-1.134,2.85-2.642,3.561-4.521c0.707-1.875,0.799-3.789,0.305-5.744
		c0.901,0,1.789,0,2.695,0C27.432,13.178,27.432,13.229,27.432,13.281z M27.432,8.321c-0.002,0.705-0.521,1.219-1.229,1.22
		c-0.009,0-0.019,0-0.024,0c-0.574,0-1.152,0-1.73,0c-0.588,0-1.178,0.001-1.768-0.001c-0.679,0-1.207-0.507-1.211-1.186
		c-0.008-1.195-0.008-2.39,0-3.583c0.004-0.687,0.53-1.19,1.221-1.191c1.176-0.001,2.349-0.001,3.523,0
		c0.696,0.001,1.217,0.518,1.219,1.218C27.434,5.974,27.434,7.147,27.432,8.321z"
              />
            </g>
          </svg>
        </g-link>

        <g-link to="https://twitter.com/SamHarlow_">
          <svg viewBox="0 0 310 310">
            <g id="XMLID_826_">
              <path
                id="XMLID_827_"
                d="M302.973,57.388c-4.87,2.16-9.877,3.983-14.993,5.463c6.057-6.85,10.675-14.91,13.494-23.73
		c0.632-1.977-0.023-4.141-1.648-5.434c-1.623-1.294-3.878-1.449-5.665-0.39c-10.865,6.444-22.587,11.075-34.878,13.783
		c-12.381-12.098-29.197-18.983-46.581-18.983c-36.695,0-66.549,29.853-66.549,66.547c0,2.89,0.183,5.764,0.545,8.598
		C101.163,99.244,58.83,76.863,29.76,41.204c-1.036-1.271-2.632-1.956-4.266-1.825c-1.635,0.128-3.104,1.05-3.93,2.467
		c-5.896,10.117-9.013,21.688-9.013,33.461c0,16.035,5.725,31.249,15.838,43.137c-3.075-1.065-6.059-2.396-8.907-3.977
		c-1.529-0.851-3.395-0.838-4.914,0.033c-1.52,0.871-2.473,2.473-2.513,4.224c-0.007,0.295-0.007,0.59-0.007,0.889
		c0,23.935,12.882,45.484,32.577,57.229c-1.692-0.169-3.383-0.414-5.063-0.735c-1.732-0.331-3.513,0.276-4.681,1.597
		c-1.17,1.32-1.557,3.16-1.018,4.84c7.29,22.76,26.059,39.501,48.749,44.605c-18.819,11.787-40.34,17.961-62.932,17.961
		c-4.714,0-9.455-0.277-14.095-0.826c-2.305-0.274-4.509,1.087-5.294,3.279c-0.785,2.193,0.047,4.638,2.008,5.895
		c29.023,18.609,62.582,28.445,97.047,28.445c67.754,0,110.139-31.95,133.764-58.753c29.46-33.421,46.356-77.658,46.356-121.367
		c0-1.826-0.028-3.67-0.084-5.508c11.623-8.757,21.63-19.355,29.773-31.536c1.237-1.85,1.103-4.295-0.33-5.998
		C307.394,57.037,305.009,56.486,302.973,57.388z"
              />
            </g>
          </svg>
        </g-link>
      </div>
    </footer>
  </div>
</template>

<static-query>
query {
  metadata {
    siteName
  }
}
</static-query>

<style lang="scss">
body {
  font-family: "Zilla Slab", -apple-system, system-ui, BlinkMacSystemFont,
    "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif;
  margin: 0;
  padding: 0;
  line-height: 1.5;
  background-color: white;
  color: black;
}

.layout {
  max-width: 1000px;
  margin: 0 auto;
  padding-left: 20px;
  padding-right: 20px;
}

.header {
  display: flex;
  justify-content: space-between;
  align-items: center;
  margin-bottom: 20px;
  height: 80px;
  font-size: 50px;
  justify-content: center;
  padding-top: 20%;
  padding-bottom: 5%;

  svg {
    fill: #2a2a2a;
  }

  img {
    width: 75px;
  }

  .logo {
    width: 150px;

    &:hover {
      fill: #2a2a2a;
      border-bottom: 5px solid #2a2a2a;
    }
  }
}

.socials {
  display: flex;
  justify-content: space-between;
  align-items: center;
  margin-bottom: 20px;
  height: 80px;
  font-size: 50px;
  justify-content: center;

  a {
    padding: 2%;
    svg {
      height: 31px;
      color: #2a2a2a;

      &:hover {
        fill: grey;
        cursor: pointer;
      }
    }
  }
}

.nav__link {
  margin-left: 20px;
}
</style>
